...
hevc::hevc_es_parser_c::cleanup() {
  ...
  while (frames_end != frame_itr) {
    ...
    if ( (HEVC_NALU_TYPE_IDR_W_RADL == idr.type) || (HEVC_NALU_TYPE_CRA_NUT == idr.type) ) {
      ...
      if (HEVC_NALU_TYPE_RADL_N != idr.type && HEVC_NALU_TYPE_RADL_R != idr.type &&
          HEVC_NALU_TYPE_RASL_N != idr.type && HEVC_NALU_TYPE_RASL_R != idr.type) {
